...
export function InnerNav() {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px", flexShrink: 0 }}>
          {user ? (
            ...
            <div style={{ position: "relative" }}>
              <button
                ...
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "var(--rose)"; }}
                onMouseLeave={(e) => { if (!profileOpen) (e.currentTarget as HTMLElement).style.borderColor = "oklch(0.88 0.02 55)"; }}
              >
                {/* Avatar */}
                <span
                  style={{
                ...
                <span style={{ fontSize: "0.82rem", fontWeight: 600, color: "var(--charcoal)", maxWidth: "100px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {user.name.split(" ")[0]}
                </span>
                <svg
                  width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="oklch(0.6 0.01 60)" strokeWidth="2.5"
              ...
              {profileOpen && (
                ...
                >
                  {/* User info */}
                  <div style={{ padding: "16px 18px", borderBottom: "1px solid oklch(0.93 0.01 60)", background: "oklch(0.975 0.012 60)" }}>
                    <div style={{ fontSize: "0.88rem", fontWeight: 700, color: "var(--charcoal)" }}>{user.name}</div>
                    <div style={{ fontSize: "0.74rem", color: "oklch(0.6 0.01 60)", marginTop: "2px" }}>
                  ...
                  {AUTH_PAGES.slice(0, 3).map(({ to, label }) => (
                    <Link
                      ...
                      style={{
                        ...
                        fontSize: "0.84rem", color: "oklch(0.45 0.01 60)", fontWeight: 500,
                        transition: "background 0.15s",
                      }}
                      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = "oklch(0.97 0.015 60)"; }}
          ...
          <button
            ...
            style={{ background: "none", border: "none", cursor: "pointer", padding: "6px", color: "var(--charcoal)" }}
          >
      ...
      {/* Mobile menu */}
      {menuOpen && (
        <div style={{ background: "oklch(0.985 0.012 60)", borderTop: "1px solid oklch(0.9 0.02 55)", padding: "12px 24px 20px" }}>
          {user && (
            <div style={{ padding: "12px 0 14px", borderBottom: "1px solid oklch(0.92 0.01 60)", marginBottom: "8px", display: "flex", alignItems: "center", gap: "10px" }}>
              <span style={{ width: "32px", height: "32px", borderRadius: "50%", background: "linear-gradient(135deg, var(--rose), var(--sunset))", display: "grid", placeItems: "center", color: "white", fontWeight: 700, fontSize: "0.75rem" }}>{initials}</span>
              <div>
                <div style={{ fontSize: "0.88rem", fontWeight: 700, color: "var(--charcoal)" }}>{user.name}</div>
                <div style={{ fontSize: "0.72rem", color: "oklch(0.6 0.01 60)" }}>{user.email || user.phone}</div>
              </div>
            </div>
          )}
          {pages.map(({ to, hash, label }) => (
          ...
          {user ? (
            <button
              ...
              style={{ marginTop: "14px", width: "100%", padding: "13px", borderRadius: "12px", border: "1.5px solid oklch(0.88 0.02 55)", background: "white", color: "oklch(0.5 0.08 18)", fontSize: "0.9rem", fontWeight: 600, cursor: "pointer", fontFamily: "var(--font-sans)" }}
            >
              Sign Out
            </button>
          ) : (

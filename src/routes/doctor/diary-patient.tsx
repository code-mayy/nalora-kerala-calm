...
export const Route = createFileRoute("/doctor/diary-patient")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Dedicated patient clinical timeline and therapeutic notes manager." },
    ],
  }),
...
const PATIENTS_DB: Record<number, {
  ...
  1: {
    id: 1, name: "Anjali Menon", age: 29, gender: "Female",
    ...
    defaultLogs: [
      { date: "2026-06-25", timestamp: "09:45 AM", author: "Dr. Aisha Menon", text: "Patient displays steady recovery. Anxiety scale down to 4/10. Sleep routine improved. Prescribed continuing light breathing exercises." },
      { date: "2026-06-20", timestamp: "10:30 AM", author: "Dr. Aisha Menon", text: "First followup. Patient reported elevated postpartum anxiety levels (anxiety scale 7/10). Advised partner integration in overnight care cycles." }
    ]
  },
  2: {
    id: 2, name: "Priyanka Raj", age: 31, gender: "Female",
    ...
    defaultLogs: [
      { date: "2026-06-25", timestamp: "12:15 PM", author: "Dr. Aisha Menon", text: "Routine review. Discussed stress factors. Advised breathing exercises and partner support. Scheduled review in 1 week." },
      { date: "2026-06-22", timestamp: "02:00 PM", author: "Dr. Aisha Menon", text: "Initial consultation. Patient experiencing severe postpartum tiredness. Lack of structural social support from remote family. Encouraged join of mother's circle group." }
    ]
  },
  3: {
    id: 3, name: "Sneha Nair", age: 27, gender: "Female",
    ...
    defaultLogs: [
      { date: "2026-06-25", timestamp: "03:00 PM", author: "Dr. Aisha Menon", text: "Acute distress due to difficult birth. Focusing on trauma release techniques and validation. Sneha showed active engagement. Will monitor weekly." }
    ]
  },
  4: {
    id: 4, name: "Divya Pillai", age: 33, gender: "Female",
  ...
  5: {
    id: 5, name: "Meera Krishnan", age: 26, gender: "Female",
...
function DoctorDiaryPatient() {
  ...
  const [logs, setLogs] = useState<{ date: string; timestamp: string; author: string; text: string }[]>([]);
  ...
  function handleAddLog(e: React.FormEvent) {
    ...
    if (typeof window !== "undefined") {
      ...
      const customOnly = updatedLogs.filter(log => 
        !info.defaultLogs.some(def => def.text === log.text && def.date === log.date)
      );
        ...
        >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
            ...
            <div style={{ background: "white", padding: "28px", borderRadius: "24px", border: "1px solid oklch(0.92 0.01 240)", boxShadow: "0 4px 20px oklch(0 0 0 / 0.01)" }}>
              <div style={{ textAlign: "center", paddingBottom: "16px", borderBottom: "1px solid oklch(0.95 0.01 240)", marginBottom: "16px" }}>
                <span
                  style={{
                    width: "56px", height: "56px", borderRadius: "50%",
                    background: "linear-gradient(135deg, oklch(0.4 0.03 240 / 0.15), oklch(0.5 0.04 200 / 0.15))",
                ...
                <h2 style={{ fontSize: "1.3rem", fontWeight: 700, color: "var(--charcoal)", margin: 0 }}>{info.name}</h2>
                <p style={{ fontSize: "0.84rem", color: "oklch(0.55 0.01 240)", margin: "4px 0 0" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "0.75rem", fontWeight: 700, color: "oklch(0.55 0.01 240)", textTransform: "uppercase", letterSpacing: "0.05em" }}>Contact Details</label>
                  <span style={{ fontSize: "0.88rem", color: "var(--charcoal)" }}>{info.email}</span>
                  <div style={{ fontSize: "0.88rem", color: "var(--charcoal)", marginTop: "2px" }}>{info.phone}</div>
                </div>
                <div>
                  ...
                  <span style={{ fontSize: "0.88rem", color: "var(--charcoal)", fontWeight: 600 }}>{info.bystanderName}</span>
                  <div style={{ fontSize: "0.82rem", color: "oklch(0.55 0.01 240)", marginTop: "2px" }}>{info.bystanderPhone}</div>
                </div>
              </div>
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "28px" }}>
            ...
            <div style={{ background: "white", padding: "32px", borderRadius: "24px", border: "1px solid oklch(0.92 0.01 240)", boxShadow: "0 4px 20px oklch(0 0 0 / 0.01)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                <h3 style={{ fontSize: "1.2rem", fontWeight: 700, color: "var(--charcoal)", margin: 0 }}>Add New Therapeutic Diary Entry</h3>
                {successMessage && <span style={{ fontSize: "0.88rem", color: "#16a34a", fontWeight: 600 }}>{successMessage}</span>}
              </div>
            ...
            {/* Timeline log */}
            <div style={{ background: "white", padding: "32px", borderRadius: "24px", border: "1px solid oklch(0.92 0.01 240)", boxShadow: "0 4px 20px oklch(0 0 0 / 0.01)" }}>
              <h3 style={{ fontSize: "1.2rem", fontWeight: 700, color: "var(--charcoal)", margin: "0 0 24px" }}>Patient Clinical Timeline</h3>
              
              {logs.length === 0 ? (
                <div style={{ textAlign: "center", padding: "40px 10px", color: "oklch(0.55 0.01 240)" }}>
                  <span style={{ fontSize: "2rem" }}>📓</span>
                  <p style={{ fontSize: "0.9rem", marginTop: "10px", margin: "10px 0 0" }}>No diary logs recorded on this file yet.</p>
                </div>
              ) : (
                <div style={{ display: "flex", flexDirection: "column", gap: "24px", position: "relative", paddingLeft: "24px" }}>
                  {/* Vertical line of timeline */}
                  <div style={{ position: "absolute", left: "7px", top: "10px", bottom: "10px", width: "1.5px", background: "oklch(0.9 0.01 240)" }} />
                    ...
                    <div key={idx} style={{ position: "relative" }}>
                      ...
                      <span
                        style={{
                          ...
                          background: "white", border: "2px solid oklch(0.4 0.03 240)",
                      ...
                      <div>
                        ...
                        <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                          <span style={{ fontSize: "0.84rem", fontWeight: 700, color: "var(--charcoal)" }}>{log.date}</span>
                          <span style={{ fontSize: "0.78rem", color: "oklch(0.55 0.01 240)" }}>{log.timestamp}</span>
                          <span style={{ fontSize: "0.76rem", fontWeight: 600, color: "oklch(0.5 0.04 200)", background: "oklch(0.96 0.01 240)", padding: "1px 6px", borderRadius: "4px" }}>
                            {log.author}
                          </span>
                        </div>

                        {/* Text */}
                        <p style={{ fontSize: "0.92rem", color: "oklch(0.3 0.01 240)", margin: "8px 0 0", lineHeight: 1.5, whiteSpace: "pre-wrap" }}>
                          {log.text}
                        </p>
                      </div>

...
export const Route = createFileRoute("/doctor/google-calendar")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Integrated clinical calendar showcasing all booked consultations." },
    ],
  }),
...
const CALENDAR_APPOINTMENTS: Record<number, { patient: string; time: string; status: string; id: number }[]> = {
  22: [
    { id: 9, patient: "Neethu Mohan", time: "04:00 PM", status: "cancelled" }
  ],
  23: [
    { id: 8, patient: "Reshma S.", time: "11:00 AM", status: "completed" }
  ],
  24: [
  ...
  29: [
    { id: 12, patient: "Dhanya K.", time: "09:30 AM", status: "upcoming" }
  ]
};
        ...
        >
          ...
          <div style={{ background: "white", padding: "32px", borderRadius: "24px", border: "1px solid oklch(0.92 0.01 240)", boxShadow: "0 4px 20px oklch(0 0 0 / 0.01)" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
              <h2 style={{ fontFamily: "var(--font-display)", fontSize: "1.25rem", fontWeight: 700, color: "var(--charcoal)", margin: 0 }}>
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button
                  style={{
                    ...
                    border: "1.5px solid oklch(0.88 0.02 240)", background: "white",
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "8px", textAlign: "center", marginBottom: "12px" }}>
              {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((day) => (
                <div key={day} style={{ fontSize: "0.72rem", fontWeight: 700, textTransform: "uppercase", color: "oklch(0.55 0.01 240)", letterSpacing: "0.04em", padding: "6px 0" }}>
                  {day}
                </div>
              ))}
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "8px" }}>
              {daysGrid.map((day, idx) => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontSize: "1rem", fontWeight: isSelected ? 700 : 500, color: isSelected ? "oklch(0.4 0.03 240)" : "var(--charcoal)" }}>
                      {day}
                    </span>
          ...
          <div style={{ background: "white", padding: "32px", borderRadius: "24px", border: "1px solid oklch(0.92 0.01 240)", boxShadow: "0 4px 20px oklch(0 0 0 / 0.01)", display: "flex", flexDirection: "column", height: "fit-content" }}>
            ...
            <div style={{ borderBottom: "1px solid oklch(0.95 0.01 240)", paddingBottom: "16px", marginBottom: "20px" }}>
              <h3 style={{ fontFamily: "var(--font-display)", fontSize: "1.15rem", fontWeight: 700, color: "var(--charcoal)", margin: 0 }}>
                Agenda Details
              </h3>
            ...
            {selectedDayAppointments.length === 0 ? (
              <div style={{ textAlign: "center", padding: "40px 10px", color: "oklch(0.55 0.01 240)" }}>
                ...
                <p style={{ fontSize: "0.82rem", marginTop: "10px", margin: "10px 0 0" }}>No bookings synced for this date.</p>
              </div>
            ) : (
                ...
                {selectedDayAppointments.map((app) => (
                  ...
                  >
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <span style={{ fontSize: "0.8rem", color: "oklch(0.5 0.04 200)", fontWeight: 700 }}>{app.time}</span>
                      
                      {app.status === "completed" && (
                        <span style={{ fontSize: "0.72rem", fontWeight: 700, padding: "1px 6px", borderRadius: "99px", background: "oklch(0.95 0.02 145)", color: "oklch(0.35 0.06 145)" }}>
                          Done
                        </span>
                      )}
                      {app.status === "in-progress" && (
                        <span style={{ fontSize: "0.72rem", fontWeight: 700, padding: "1px 6px", borderRadius: "99px", background: "oklch(0.95 0.02 240)", color: "oklch(0.45 0.08 240)" }}>
                          Session
                        </span>
                      )}
                      {app.status === "upcoming" && (
                        <span style={{ fontSize: "0.72rem", fontWeight: 700, padding: "1px 6px", borderRadius: "99px", background: "oklch(0.96 0.01 240)", color: "oklch(0.5 0.01 240)" }}>
                          Upcoming
                        </span>
                      )}
                    </div>

                    <h4 style={{ fontSize: "1.05rem", fontWeight: 600, color: "var(--charcoal)", margin: "8px 0 2px" }}>
                      {app.patient}
                    </h4>
